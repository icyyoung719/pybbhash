...
def main():
    ...
    if verification_passed:
        ...
        if len(unique_indices) == len(test_keys):
            ...
            if sorted(mapped_indices) == list(range(len(test_keys))):
                print(f"        ✓ Sampled indices form a perfect permutation [0, {len(test_keys)-1}]")
            else:
                print(f"        ℹ Sampled indices don't form a complete permutation (expected for subset)")
    else:
    ...
    print(f"      ✓ Saved MPHF to '{OUTPUT_FILE}' ({file_size:,} bytes, {file_size/len(keys):.2f} bytes/key)")
    ...
    for key in test_keys:
        ...
        if original_index != loaded_index:
            print(f"        ✗ ERROR: Key {key} mapped to {loaded_index} after load, expected {original_index}")
