...
def main():
    ...
    CSV_PATH = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "res", "test_data.csv"
    )
    GAMMA = 2.0
    ...
    for i in range(min(5, len(composite_keys))):
        ...
        print(
            f"        [{i}] uint64=0x{composite_keys[i]:016X} "
    ...
    test_indices = random.sample(
        range(len(composite_keys)), min(NUM_TEST_LOOKUPS, len(composite_keys))
    )
    ...
    for i, idx in enumerate(test_indices[:10]):
        ...
        print(
            f"        [{i}] Key(0x{high:08X}, 0x{low:08X}) -> "
    ...
    print(
        f"      Total size: {total_size:,} bytes ({total_size/len(composite_keys):.2f} bytes/entry)"
    )
    ...
    for idx in test_indices[:10]:
        ...
        if abs(loaded_value - expected_value) > 1e-9 or abs(loaded_value - original_value) > 1e-9:
            ...
            print(
                f"        ✗ Error: Key(0x{high:08X}, 0x{low:08X}) -> "
